...
//Custom by Jacky 12/9
half3 ShadeHighLight(ToonSurfaceData surfaceData, ToonLightingData lightingData, Light light)
{
    // half vector: light direction and view direction
    half3 halfVector = normalize(light.direction + lightingData.viewDirectionWS);
    half specular =  dot(halfVector, lightingData.normalWS);
    half specularLight = lerp(0, 1, specular);
    specularLight = smoothstep(0.5 + _SpecularLightAmount - 0.001, 0.5 + _SpecularLightAmount + 0.001, specularLight);
    specularLight *= lerp(1,light.shadowAttenuation,_ReceiveShadowMappingAmount);
    return saturate(light.color) * specularLight;
}
...
half3 CompositeAllLightResults(half3 indirectResult, half3 mainLightResult, half3 additionalLightSumResult, half3 emissionResult, half3 highLightResult, ToonSurfaceData surfaceData, ToonLightingData lightingData)
{
    ...
    half3 rawLightSum = max(indirectResult, mainLightResult + additionalLightSumResult + highLightResult); // pick the highest between indirect and direct light
